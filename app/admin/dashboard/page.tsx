...
import { Plus, ExternalLink, Pencil, FileText, TrendingUp, Zap } from "lucide-react";
    ...
    <div className="px-8 py-8 w-full">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">Manage your newsletter posts</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        ...
        <div className="p-6 rounded-2xl bg-white border border-slate-200 shadow-sm flex items-start gap-4">
          ...
          <div>
            <p className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-1">Latest Issue</p>
            <p className="text-sm font-bold text-slate-700 truncate max-w-[160px]">{latest?.title ?? "—"}</p>
            {latest && <p className="text-slate-400 text-xs mt-0.5">{new Date(latest.date).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</p>}
          </div>
        </div>
        <div className="p-6 rounded-2xl bg-blue-600 border border-blue-500 shadow-md shadow-blue-200 flex items-start gap-4">
          <div className="w-10 h-10 rounded-xl bg-white/20 flex items-center justify-center shrink-0">
            <Zap className="w-5 h-5 text-white fill-white" />
          </div>
          <div>
            <p className="text-xs font-bold uppercase tracking-widest text-blue-200 mb-1">Quick Action</p>
            <Link href="/admin/posts/new" className="text-sm font-bold text-white hover:text-blue-100 transition-colors">
              Write new issue →
            </Link>
          </div>
        </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <h2 className="text-sm font-black text-slate-700 uppercase tracking-widest">Recent Posts</h2>
          <Link href="/admin/posts" className="text-xs font-bold text-blue-500 hover:text-blue-700 transition-colors">View all →</Link>
        </div>
        ...
        {posts.length === 0 ? (
          ...
          <div>
            ...
            {posts.slice(0, 5).map((post, i) => (
              <div key={post.slug} className={`grid grid-cols-12 gap-4 px-6 py-4 items-center hover:bg-slate-50 transition-colors ${i !== Math.min(posts.length, 5) - 1 ? "border-b border-slate-100" : ""}`}>
