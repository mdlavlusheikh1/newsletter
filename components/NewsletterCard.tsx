    ...
    <Link href={`/newsletter/${slug}`} className="group block h-full">
      <article className="h-full flex flex-col p-6 rounded-2xl bg-white border border-slate-100 hover:border-blue-200 hover:shadow-md hover:shadow-blue-50 transition-all duration-200">
        ...
        <div className="flex items-center justify-between mb-4">
          <span className="px-2.5 py-1 text-[11px] font-bold rounded-lg bg-blue-50 text-blue-600">
            {tags[0] ?? "Issue"}
          </span>
          <span className="text-[11px] text-slate-400 font-medium">5 min read</span>
        </div>
        ...
        <div className="flex items-center justify-between pt-4 border-t border-slate-100">
          ...
          <span className="text-blue-500 text-sm font-semibold group-hover:translate-x-0.5 transition-transform">
            Read →
          </span>
        </div>
      </article>
